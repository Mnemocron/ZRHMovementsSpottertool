...
try:
	...
	from datetime import date, timedelta 				# time formats
...
def getZRH(ftyp):
	if ftyp is 'Arrival':
		fname = 'timetable.arrival.tom'
	elif ftyp is 'Departure':
		fname = 'timetable.departure.tom'
	else:
		return
	...
	r_dtod = date.today()
	r_date = str(r_dtod)				# set date - 'yyyy-mm-dd'
	r_dtom = r_dtod + timedelta(days=1)
	r_date2 = str(r_dtom) 
	r_now  = dt.datetime.now().hour
	...
	for spt in range(2):				# standard and spotter
		...
		while not lastFlightFetched :	# fetch every flight by scrolling through pages
			...
			body = urllib.urlencode({'startDateTime' : r_date2 + 'T' + r_time2 + '.000Z', 'search' : r_sear, 'page' : r_page, '__RequestVerificationToken' : ''})
			# print (body) 				# debug
